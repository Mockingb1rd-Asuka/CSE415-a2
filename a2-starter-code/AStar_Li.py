...
email: l0000000@uw.edu
Assignment 3, in CSE 415, Spring 2019.
This file contains the algorithm of A* and finds the shortest path from
one location to the other. It defaults in using hamming's distance to
solve the puzzle.
...
import importlib
#Problem = importlib.import_module("EightPuzzleWithManhattan")
#Problem = importlib.import_module("EightPuzzleWithHamming")
#importlib.import_module("EightPuzzle") as Problem

if sys.argv == [''] or len(sys.argv) < 2:
    import EightPuzzle as Problem
else:
    import importlib as Problem
...
# STEP 2. If OPEN is empty, output “DONE” and stop.
  while OPEN.__len__() != 0: # ***STUDENTS CHANGE THIS CONDITION***                    CHANGED
...
#         If S is a goal state, output its description
    ...
    if Problem.GOAL_TEST(S):
      ...
      return backtrace(S)
      # ***STUDENTS CHANGE THE BODY OF THIS IF.***                  CHANGED
...
#         and if it is already on CLOSED, delete the new instance.
    ...
    for op in Problem.OPERATORS:
      if op.precond(S):
        ...
        new_state_weight = S.edge_distance(new_state) + g[S]
        new_f = new_state_weight + h(new_state)
        if OPEN.__contains__(new_state) and g[new_state] > new_state_weight:
            g[new_state] = new_state_weight
            OPEN.__delitem__(new_state)

            f[new_state] = new_f
            OPEN.insert(new_state, new_f)
            BACKLINKS[new_state] = S
        elif CLOSED.__contains__(new_state) and g[new_state] > new_state_weight and new_f > f[new_state]:
            g[new_state] = new_state_weight
            CLOSED.remove(new_state)

            OPEN.insert(new_state, new_f)
            BACKLINKS[new_state] = S
        elif not new_state in CLOSED and not OPEN.__contains__(new_state):
            g[new_state] = new_state_weight
            f[new_state] = new_f
